...
export class PostDownloader extends Downloader {
  ...
  private static async downloadWithSlider(element: HTMLElement): Promise<void> {
    ...
    const sliderItems = [
      ...element.querySelectorAll(QuerySelectors.sliderItem),
    ] as HTMLElement[];
  ...
  private static async download(
    ...
    if (img) {
      ...
      if (currentSrc?.startsWith?.('blob:')) {
        Alert.createAndAdd(
          'Videos cannot be downloaded, because IG started blocking it.',
          'warn',
        );
        return;
      }
    ...
    const postAccountName =
      (element.querySelector(
        QuerySelectors.postAccountName,
      ) as HTMLElement | null)?.innerText || 'unknown';
  ...
  private static async downloadWithOutSlider(
    element: HTMLElement,
  ): Promise<void> {
    const postContentWrapper =
      element.querySelector(QuerySelectors.postContentWrapper) ||
  ...
  public async createDownloadButton(): Promise<void> {
    let postList: HTMLElement[] = [
      ...document.querySelectorAll(QuerySelectors.postWrapper),
    ] as HTMLElement[];
  ...
  private async retryCreateButton(
    maxRetries: number = 20,
    retries: number = 0,
  ): Promise<HTMLElement[]> {
    await new Promise((resolve) => {
      this.creationTimeoutList.push(
        (setTimeout(resolve, 100) as unknown) as number,
      );
    });
    let postList = [
      ...document.querySelectorAll(QuerySelectors.postWrapper),
  ...
  private addDownloadButton(element: HTMLElement): void {
    // Only first post
    const bookmarkElement: HTMLElement = element.querySelector(
      QuerySelectors.postBookmark,
    ) as HTMLElement;
    const downloadButton: HTMLElement = document.createElement('span');
    // Get the value of the background color variable (will be either 255,255,255 or 0,0,0)
    const backgroundColor = getComputedStyle(
      document.documentElement,
    ).getPropertyValue('--ig-primary-background');

    if (backgroundColor === '0, 0, 0') {
      downloadButton.style.backgroundImage = 'var(--extension-download-white)';
    } else {
      downloadButton.style.backgroundImage = 'var(--extension-download-black)';
    }
